...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_prefix=r"D:\mmclassification\Five_pics\train",  # 训练集路径 
        pipeline=train_pipeline, #数据集 需要经过的数据流水线
    ),
    val=dict(
        type=dataset_type,
        #data_prefix="train_dataset\\train",
        data_prefix=r"D:\mmclassification\Five_pics\val", #验证集路径
        pipeline=test_pipeline,
    ),
    test=dict(
        type=dataset_type,
        data_prefix=r"D:\mmclassification\Five_pics\train",
        pipeline=test_pipeline,
    ),
)
...
model = dict(
    ...
    backbone=dict(type="VGG", depth=16, norm_cfg=dict(type="BN"), num_classes=2),# VGG是backnone（主干）网络类型；16层；norm layer的类型是BN；输出类别数是2类，与数据集一致
    neck=None,
    head=dict(
        type="ClsHead",# 对应上面的head类
        loss=dict(type="CrossEntropyLoss", loss_weight=1.0),
        topk=(1,),# 评估指标,Top-k准确率,这里为 top1 准确率
    ),
)
...
work_dir = "configs/org/work_dirs/vgg16_lung_20220831test_org"
